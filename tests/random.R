...
# no regularisation
params <- list(nthread = 8, max_depth = 5, eta = 0.05, subsample = 0.5, colsample_bytree = 0.5, min_child_weight = 1, lambda = 0, alpha = 0, tree_method = "hist", objective = "reg:squarederror", eval_metric = "rmse", max_bin=32)
...
params <- list(nthread = 8, max_depth = 5, eta = 0.05, subsample = 0.5, colsample_bytree = 0.5, min_child_weight = 1, lambda = 0, alpha = 0, tree_method = "gpu_hist", objective = "reg:squarederror", eval_metric = "rmse", max_bin=32)
system.time(xgb_train <- xgb.DMatrix(data = data_train, label = target_train))
system.time(model <- xgb.train(data = xgb_train, params = params, nrounds = 100, verbose = 1, print_every_n = 2L, early_stopping_rounds = NULL))
pred_xgb <- predict(model, xgb_train)

# evotrees
params <- list(loss = "linear", nrounds = 100, eta = 0.05, lambda = 0, gamma = 0.01, max_depth = 6, min_weight = 1, rowsample = 0.5, colsample = 0.5, nbins = 32, metric="mse")
...
# no regularisation
params <- list(nthreads = 4, max_depth = 5, eta = 0.05, subsample = 0.5, colsample_bytree = 0.5, min_child_weight = 1, lambda = 0, alpha = 0, tree_method = "hist", objective = "multi:softprob", num_class=3, eval_metric = "mlogloss", max_bin=32)
